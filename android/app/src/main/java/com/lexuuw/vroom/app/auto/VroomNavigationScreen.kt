...
class VroomNavigationScreen(carContext: CarContext) : Screen(carContext) {
  ...
  /**
   * We keep a minimal action strip (app icon) so the NavigationTemplate builds reliably on all
   * host versions. Action.APP_ICON shows the VROOM logo — no gray "back" button.
   * All real navigation (menu, search, report, settings) is handled on the custom canvas overlay.
   */
