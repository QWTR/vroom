...
import { ActivityIndicator, Alert, RefreshControl, ScrollView, Share, TouchableOpacity, View } from 'react-native';
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.bg }}>
      <View style={{ paddingTop: top, paddingHorizontal: 16, paddingBottom: 13, borderBottomWidth: 1, borderBottomColor: theme.border, flexDirection: 'row', alignItems: 'center', gap: 12 }}>
        ...
        <View style={{ flex: 1 }}><Text style={{ color: theme.text, fontFamily: 'Manrope_600SemiBold', fontWeight: '900', fontSize: 15, letterSpacing: 1 }}>PROGRAM POLECEŃ</Text><Text style={{ color: theme.textDim, marginTop: 3, fontSize: 12 }}>{dashboard.qualifiedCount} ważnych zaproszeń</Text></View>
      </View>
      ...
      <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void load(true)} tintColor={theme.primary} />} contentContainerStyle={{ padding: 16, paddingBottom: 44, gap: 14 }}>
        <View style={{ ...card, padding: 16 }}>
          <Text style={{ color: theme.textDim, fontFamily: 'Manrope_600SemiBold', fontSize: 12, letterSpacing: 1 }}>TWÓJ KOD</Text>
          <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 8, gap: 10 }}>
            <Text selectable style={{ flex: 1, color: theme.primary, fontFamily: 'Manrope_600SemiBold', fontWeight: '900', fontSize: 22, letterSpacing: 1 }}>{dashboard.code}</Text>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
          ...
          ].map(([label, value]) => <View key={String(label)} style={{ ...card, width: '48%', padding: 14 }}><Text style={{ color: theme.textDim, fontFamily: 'Manrope_600SemiBold', fontSize: 12, letterSpacing: 1 }}>{label}</Text><Text style={{ color: Number(value) < 0 ? '#ff5368' : theme.text, fontFamily: 'Manrope_600SemiBold', fontWeight: '900', fontSize: 16, marginTop: 8 }}>{money(Number(value))}</Text></View>)}
        </View>
        ...
        <View style={{ ...card, padding: 16 }}>
          ...
          {!!dashboard.nextTier && <Text style={{ color: theme.textDim, fontSize: 12, marginTop: 9 }}>Do kolejnego progu brakuje {Math.max(0, dashboard.nextTier.inviteCount - dashboard.qualifiedCount)} osób · łączny zarobek {money(dashboard.nextTier.targetAmount)}</Text>}
        </View>

        <View style={{ ...card, overflow: 'hidden' }}>
          <TouchableOpacity onPress={() => setShowPayout((value) => !value)} style={{ padding: 16, flexDirection: 'row', alignItems: 'center', gap: 10 }}><MaterialCommunityIcons name="bank-transfer" size={24} color={theme.primary} /><View style={{ flex: 1 }}><Text style={{ color: theme.text, fontWeight: '800' }}>Wypłata środków</Text><Text style={{ color: theme.textDim, fontSize: 12, marginTop: 2 }}>Minimum {money(dashboard.payoutMinimum)}</Text></View><MaterialIcons name={showPayout ? 'expand-less' : 'expand-more'} size={22} color={theme.textDim} /></TouchableOpacity>
        ...
        {!!dashboard.ledger?.length && <View style={{ ...card, padding: 16 }}>
          <Text style={{ color: theme.text, fontFamily: 'Manrope_600SemiBold', fontWeight: '800', fontSize: 12, letterSpacing: 1 }}>HISTORIA SALDA</Text>
          {dashboard.ledger.slice(0, 8).map((entry: any) => <View key={entry.id} style={{ flexDirection: 'row', gap: 10, paddingTop: 11, marginTop: 11, borderTopWidth: 1, borderTopColor: theme.border }}><View style={{ flex: 1 }}><Text style={{ color: theme.text, fontWeight: '700', fontSize: 12 }}>{entry.type === 'award' || entry.type === 'opening' ? 'Nagroda za próg' : entry.type === 'correction' ? 'Korekta zaproszeń' : entry.type === 'payout_reserve' ? 'Rezerwacja wypłaty' : entry.type === 'payout_release' ? 'Zwolnienie rezerwacji' : entry.type}</Text><Text style={{ color: theme.textDim, fontSize: 12, marginTop: 3 }}>{new Date(entry.createdAt).toLocaleDateString('pl-PL')}</Text></View><Text style={{ color: Number(entry.amount) < 0 ? '#ff5368' : '#4de926', fontWeight: '900' }}>{Number(entry.amount) > 0 ? '+' : ''}{money(entry.amount)}</Text></View>)}
        </View>}

        {!!dashboard.payouts?.length && <View style={{ ...card, padding: 16 }}>
          <Text style={{ color: theme.text, fontFamily: 'Manrope_600SemiBold', fontWeight: '800', fontSize: 12, letterSpacing: 1 }}>HISTORIA WYPŁAT</Text>
          {dashboard.payouts.slice(0, 8).map((payout: any) => <View key={payout.id} style={{ flexDirection: 'row', gap: 10, paddingTop: 11, marginTop: 11, borderTopWidth: 1, borderTopColor: theme.border, alignItems: 'center' }}><View style={{ flex: 1 }}><Text style={{ color: theme.text, fontWeight: '700', fontSize: 12 }}>{payout.status === 'requested' ? 'Oczekuje' : payout.status === 'approved' ? 'Zatwierdzona' : payout.status === 'paid' ? 'Wypłacona' : payout.status === 'rejected' ? 'Odrzucona' : 'Anulowana'}</Text><Text style={{ color: theme.textDim, fontSize: 12, marginTop: 3 }}>{new Date(payout.requestedAt).toLocaleDateString('pl-PL')} · konto •••• {payout.ibanLast4}</Text></View><View style={{ alignItems: 'flex-end', gap: 5 }}><Text style={{ color: theme.text, fontWeight: '900' }}>{money(payout.amount)}</Text>{payout.status === 'requested' && <TouchableOpacity disabled={saving} onPress={() => void cancelPayout(payout.id)}><Text style={{ color: '#ff5368', fontSize: 12, fontWeight: '800' }}>ANULUJ</Text></TouchableOpacity>}</View></View>)}
        </View>}

        <Text style={{ color: theme.text, fontFamily: 'Manrope_600SemiBold', fontWeight: '800', fontSize: 12, letterSpacing: 1, marginTop: 4 }}>ZAPROSZENI ({dashboard.qualifiedCount})</Text>
        {invitees.length === 0 ? <View style={{ ...card, padding: 24, alignItems: 'center' }}><Text style={{ color: theme.textDim }}>Nikt jeszcze nie użył Twojego kodu.</Text></View> : invitees.map((item) => (
          <TouchableOpacity key={item.id} disabled={!item.user} onPress={() => item.user && router.push(`/profile/${item.user.id}` as any)} style={{ ...card, padding: 13, flexDirection: 'row', gap: 11, alignItems: 'center', opacity: item.status === 'invalid' ? 0.55 : 1 }}>
            ...
            <View style={{ flex: 1 }}><Text style={{ color: theme.text, fontWeight: '800' }}>{item.user?.username || 'Konto usunięte'}</Text><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 5, marginTop: 6 }}>{item.user?.premiumSources?.map((source, index) => <Text key={`${source.type}-${index}`} style={{ color: source.type === 'purchase' ? '#ffd166' : source.type === 'welcome' ? '#4de926' : '#7bb6ff', backgroundColor: theme.bg, borderRadius: 6, paddingHorizontal: 6, paddingVertical: 3, fontSize: 12, fontWeight: '800' }}>{premiumLabel(source)}</Text>)}{!item.user?.premiumSources?.length && <Text style={{ color: theme.textDim, fontSize: 12 }}>Bez premium</Text>}</View></View>
            {dashboard.showInviteeSpendStatusInApp === true && <View style={{ alignItems: 'flex-end', gap: 5 }}><MaterialCommunityIcons name={item.hasPaidSpend ? 'cash-check' : 'cash-remove'} size={20} color={item.hasPaidSpend ? '#4de926' : theme.textDim} /><Text style={{ color: item.hasPaidSpend ? '#4de926' : theme.textDim, fontSize: 12 }}>{item.hasPaidSpend ? 'ZAPŁACIŁ' : 'BRAK PŁATNOŚCI'}</Text></View>}
          </TouchableOpacity>
        ))}
