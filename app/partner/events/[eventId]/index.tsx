    ...
    <View style={{ flex: 1, backgroundColor: theme.bg }}>
      ...
      {loading ? (
        ...
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 28, gap: 14 }}>
          ...
          <Text style={{ color: theme.text, textAlign: 'center', fontFamily: 'Orbitron' }}>Ta treść nie jest już dostępna</Text>
          <TouchableOpacity onPress={() => router.replace('/(tabs)/community' as any)} style={{ backgroundColor: theme.primary, borderRadius: 12, paddingHorizontal: 18, paddingVertical: 11 }}>
            <Text style={{ color: '#fff', fontFamily: 'Orbitron', fontSize: 10 }}>WRÓĆ DO SPOŁECZNOŚCI</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
          ...
          <Text style={{ color: theme.text, fontFamily: 'Orbitron', fontSize: 21, fontWeight: '800' }}>{event.title}</Text>
          <Text style={{ color: theme.primary, marginTop: 7, fontFamily: 'Orbitron', fontSize: 10 }}>{event.businessAccount?.companyName || 'VROOM Partner'}</Text>
          <View style={{ marginTop: 18, padding: 15, borderRadius: 16, backgroundColor: theme.surface, borderWidth: 1, borderColor: theme.border, gap: 10 }}>
            ...
            {event.isRegistered ? <Text style={{ color: '#4de926', fontFamily: 'Orbitron', fontSize: 10 }}>JESTEŚ ZAPISANY/A</Text> : null}
          </View>
          ...
          {event.externalUrl ? (
            <TouchableOpacity onPress={() => void Linking.openURL(event.externalUrl!)} style={{ marginTop: 20, backgroundColor: theme.primary, borderRadius: 13, alignItems: 'center', paddingVertical: 13 }}>
              <Text style={{ color: '#fff', fontFamily: 'Orbitron', fontSize: 10 }}>OTWÓRZ STRONĘ WYDARZENIA</Text>
            </TouchableOpacity>
          ) : null}
        </ScrollView>
      )}
