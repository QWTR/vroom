...
export default function PartnerHubScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (detailData.myReview) {
        setReviewRating(detailData.myReview.rating);
        setReviewComment(detailData.myReview.comment || '');
      }
  ...
  const submitReview = async () => {
    ...
    setMessage('');
    try {
      await authFetch(`/api/partner-pois/${partnerId}/reviews`, {
        method: 'POST',
        body: JSON.stringify({ rating: reviewRating, comment: reviewComment.trim() || null }),
      });
      setReviewOpen(false);
      setMessage(detail?.myReview ? 'Opinia została zaktualizowana.' : 'Opinia została opublikowana.');
      await load();
    } catch (requestError: any) {
      setMessage(requestError.message);
    } finally {
    ...
    <View style={{ flex: 1, backgroundColor: theme.bg }}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 7 }}>
          {tabs.map((item) => (
            <TouchableOpacity
              ...
              style={{
                paddingHorizontal: 13, height: 38, borderRadius: 11, borderWidth: 1,
                borderColor: tab === item.id ? accent : border,
                backgroundColor: tab === item.id ? `${accent}20` : card,
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 36, gap: 12 }} showsVerticalScrollIndicator={false}>
          ...
          <>
            <View style={{ padding: 18, borderRadius: 18, borderWidth: 1, borderColor: border, backgroundColor: card }}>
              <View style={{ flexDirection: 'row', gap: 14, alignItems: 'center' }}>
                {poi?.logoUrl ? (
                  <TouchableOpacity onPress={() => setLightboxUri(normalizeMediaUri(poi.logoUrl))}>
                    <Image source={normalizeMediaUri(poi.logoUrl) || undefined} style={{ width: 62, height: 62, borderRadius: 16 }} contentFit="contain" />
                  </TouchableOpacity>
                ) : (
                  <View style={{ width: 62, height: 62, borderRadius: 16, backgroundColor: `${accent}20`, alignItems: 'center', justifyContent: 'center' }}>
                    <MaterialCommunityIcons name="store" size={29} color={accent} />
                  </View>
                )}
                <View style={{ flex: 1 }}>
                  ...
                  {detail?.stats?.reviewCount > 0 && (
                    <Text style={{ color: '#f5b942', marginTop: 6, fontWeight: '700' }}>
                      {'★'.repeat(Math.round(detail.stats.averageRating || 0))} {Number(detail.stats.averageRating || 0).toFixed(1)} · {detail.stats.reviewCount} opinii
                    </Text>
                  )}
                </View>
              </View>
            ...
            <Action icon="navigation-variant-outline" label="Nawiguj" disabled={!Number.isFinite(poi?.lat) || !Number.isFinite(poi?.lng)} onPress={openMaps} color={accent} border={border} card={card} text={theme.text} />
            ...
            {detail?.gallery?.length > 0 && (
              <ScrollView horizontal contentContainerStyle={{ gap: 10 }}>
                {detail.gallery.map((item: any) => {
                  const uri = normalizeMediaUri(item.imageUrl);
                  return (
                    <TouchableOpacity key={item.id} onPress={() => setLightboxUri(uri)}>
                      <Image source={uri || undefined} style={{ width: 250, height: 150, borderRadius: 14, backgroundColor: card }} contentFit="cover" />
                    </TouchableOpacity>
                  );
                })}
              </ScrollView>
        ...
        {tab === 'offers' && (hub?.offers?.length
          ? hub.offers.map((offer: Offer) => (
            <View key={offer.id} style={{ borderWidth: 1, borderColor: border, backgroundColor: card, borderRadius: 18, overflow: 'hidden' }}>
              {offer.imageUrl && (
                <TouchableOpacity onPress={() => setLightboxUri(normalizeMediaUri(offer.imageUrl))}>
                  <Image source={normalizeMediaUri(offer.imageUrl) || undefined} style={{ width: '100%', height: 170 }} contentFit="cover" />
                </TouchableOpacity>
              )}
        ...
        {tab === 'events' && (hub?.events?.length
          ? hub.events.map((event: PartnerEvent) => (
            <TouchableOpacity
              key={event.id}
              activeOpacity={0.86}
              onPress={() => router.push({ pathname: '/Community/meets/meet', params: { id: String(event.id) } })}
              style={{ borderWidth: 1, borderColor: border, backgroundColor: card, borderRadius: 18, overflow: 'hidden' }}
            >
              {event.coverImageUrl && (
                <TouchableOpacity onPress={() => setLightboxUri(normalizeMediaUri(event.coverImageUrl))}>
                  <Image source={normalizeMediaUri(event.coverImageUrl) || undefined} style={{ width: '100%', height: 170 }} contentFit="cover" />
                </TouchableOpacity>
              )}
        ...
        {tab === 'reviews' && (
          <>
            <TouchableOpacity
              ...
              style={{ padding: 15, borderRadius: 14, backgroundColor: accent, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', gap: 8 }}
            >
              <MaterialCommunityIcons name="star-plus-outline" size={20} color="#050505" />
              <Text style={{ color: '#050505', fontWeight: '900' }}>{detail?.myReview ? 'EDYTUJ SWOJĄ OPINIĘ' : 'WYSTAW OPINIĘ'}</Text>
            </TouchableOpacity>
            {detail?.reviews?.length
              ? detail.reviews.map((review: any) => (
                <View key={review.id} style={{ padding: 15, borderRadius: 15, borderWidth: 1, borderColor: border, backgroundColor: card }}>
      ...
      <Modal visible={leadOpen} transparent animationType="slide" onRequestClose={() => setLeadOpen(false)}>
        <Pressable onPress={() => setLeadOpen(false)} style={{ flex: 1, backgroundColor: '#000a', justifyContent: 'flex-end' }}>
          <Pressable onPress={(event) => event.stopPropagation()} style={{ padding: 20, paddingBottom: insets.bottom + 20, borderTopLeftRadius: 24, borderTopRightRadius: 24, backgroundColor: theme.surface, gap: 12 }}>
            ...
            {([['contactName', 'Imię i nazwisko'], ['contactEmail', 'Email'], ['contactPhone', 'Telefon']] as const).map(([key, placeholder]) => (
              <TextInput key={key} value={lead[key]} onChangeText={(value) => setLead((current) => ({ ...current, [key]: value }))} placeholder={placeholder} placeholderTextColor="#66717d" style={{ color: theme.text, borderWidth: 1, borderColor: border, backgroundColor: card, borderRadius: 12, padding: 13 }} />
            ))}
            ...
            <TouchableOpacity disabled={!lead.consent} onPress={sendLead} style={{ padding: 15, borderRadius: 13, backgroundColor: '#ff3b3f', alignItems: 'center', opacity: lead.consent ? 1 : 0.45 }}>
      ...
      <Modal visible={reviewOpen} transparent animationType="slide" onRequestClose={() => setReviewOpen(false)}>
        <Pressable onPress={() => setReviewOpen(false)} style={{ flex: 1, backgroundColor: '#000a', justifyContent: 'flex-end' }}>
          <Pressable onPress={(event) => event.stopPropagation()} style={{ padding: 20, paddingBottom: insets.bottom + 20, borderTopLeftRadius: 24, borderTopRightRadius: 24, backgroundColor: theme.surface, gap: 14 }}>
            <Text style={{ color: theme.text, fontSize: 20, fontWeight: '900' }}>{detail?.myReview ? 'Edytuj opinię' : 'Wystaw opinię'}</Text>
            <View style={{ flexDirection: 'row', gap: 8, justifyContent: 'center' }}>
              {[1, 2, 3, 4, 5].map((value) => (
                <TouchableOpacity key={value} onPress={() => setReviewRating(value)}>
                  <MaterialCommunityIcons name={value <= reviewRating ? 'star' : 'star-outline'} size={34} color="#f5b942" />
                </TouchableOpacity>
              ))}
            </View>
            <TextInput
              ...
              placeholder="Napisz kilka słów o wizycie (opcjonalnie)"
              placeholderTextColor="#66717d"
              multiline
              style={{ minHeight: 100, color: theme.text, borderWidth: 1, borderColor: border, backgroundColor: card, borderRadius: 12, padding: 13, textAlignVertical: 'top' }}
            />
            <TouchableOpacity disabled={reviewSaving} onPress={submitReview} style={{ padding: 15, borderRadius: 13, backgroundColor: accent, alignItems: 'center', opacity: reviewSaving ? 0.6 : 1 }}>
              <Text style={{ color: '#050505', fontWeight: '900' }}>{detail?.myReview ? 'ZAKTUALIZUJ' : 'OPUBLIKUJ'}</Text>
            </TouchableOpacity>
          </Pressable>
        </Pressable>
      </Modal>
      ...
      <Modal visible={Boolean(lightboxUri)} transparent animationType="fade" onRequestClose={() => setLightboxUri(null)}>
        <Pressable onPress={() => setLightboxUri(null)} style={{ flex: 1, backgroundColor: '#000d', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <TouchableOpacity onPress={() => setLightboxUri(null)} style={{ position: 'absolute', top: insets.top + 12, right: 16, width: 40, height: 40, borderRadius: 20, backgroundColor: '#ffffff22', alignItems: 'center', justifyContent: 'center' }}>
            <MaterialCommunityIcons name="close" size={22} color="#fff" />
          </TouchableOpacity>
          {lightboxUri ? (
            <Image
              source={lightboxUri}
              style={{ width: Dimensions.get('window').width - 24, height: Dimensions.get('window').height * 0.72 }}
              contentFit="contain"
            />
          ) : null}
        </Pressable>
...
function Notice({ text, danger = false }: { text: string; danger?: boolean }) {
  return (
    <View style={{ padding: 12, borderRadius: 12, backgroundColor: danger ? '#ff5a6318' : '#43d17b18', borderWidth: 1, borderColor: danger ? '#ff5a6340' : '#43d17b40' }}>
...
function Action({ icon, label, onPress, disabled, color, border, card, text }: any) {
  return (
    <TouchableOpacity disabled={disabled} onPress={onPress} style={{ padding: 15, borderRadius: 14, borderWidth: 1, borderColor: border, backgroundColor: card, flexDirection: 'row', alignItems: 'center', gap: 11, opacity: disabled ? 0.4 : 1 }}>
