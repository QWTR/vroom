...
/** ~4× NAV_PROGRESS_UI_MS — potwierdzenie off-route na snapped pozycji. */
const OFF_ROUTE_CONFIRM_STREAK = 4;
...
const TRIP_COMPASS_HEADING_MAX_KMH = 5;
...
function resolveDrivingHeading(
  ...
  if (kmh >= TRIP_COMPASS_HEADING_MAX_KMH) {
    ...
    if (appliedSnap.snapped && Number.isFinite(appliedSnap.targetHeading)) {
      const roadHeading = alignBearingToReference(appliedSnap.targetHeading, travelRef);
...
function resolveUnifiedHeading(params: {
  ...
  if (speedKmh >= TRIP_COMPASS_HEADING_MAX_KMH) {
    if (speedKmh < 20) {
      // Niska prędkość miejska: miks 50/50 ruch + droga, bez kompasu urządzenia.
      if (alignedMove != null) {
        target = smoothHeading(target, alignedMove, 0.5, 30);
      }
      if (alignedSnap != null) {
        target = smoothHeading(target, alignedSnap, 0.5, 24);
      }
    } else {
      if (alignedMove != null) {
        target = smoothHeading(target, alignedMove, 0.72, 32);
      }
      if (alignedSnap != null && alignedMove != null) {
        const roadDiff = Math.abs(((alignedSnap - alignedMove + 540) % 360) - 180);
        if (roadDiff <= 28) {
          target = smoothHeading(target, alignedSnap, 0.28, 22);
        }
      } else if (alignedSnap != null && alignedMove == null) {
        target = smoothHeading(target, alignedSnap, 0.45, 22);
      }
    }
  } else if (speedKmh >= 30 && alignedSnap != null) {
    target = smoothHeading(target, alignedSnap, 0.95, 22);
    if (alignedMove != null) {
      target = smoothHeading(target, alignedMove, 0.12, 18);
    }
    if (alignedGps != null) {
      target = smoothHeading(target, alignedGps, 0.06, 18);
    }
  } else {
    if (alignedMove != null) {
...
const ACTIVE_UI_LOCATION_THROTTLE_MS = 1200;
...
export default function MapScreen() {
  ...
  /** Kamera: jeden native setCamera na tick applyTripPosition (nie 60 FPS z workletu). */
  ...
  const resolveTripTravelHeading = useCallback((
    ...
    if (prev.initialized && Number.isFinite(prev.lat)) {
      movedM = haversineKm(prev.lat, prev.lng, lat, lng) * 1000;
      moveBearing = moveBearingBetween(prev.lat, prev.lng, lat, lng, movedM);
    }
    ...
    travelHeadingStateRef.current = { lat, lng, hdg, initialized: true };
    return hdg;
  }, []);
  /** Na drodze: heading z tangensa polilinii (płynny DR). Poza drogą: wektor ruchu. */
  const headingForDriveMarker = useCallback((
    ...
    return resolveTripTravelHeading(lat, lng, pose.heading, speedKmh);
  ...
  /** V2: marker 60 FPS przez DriveMarkerLayer + useDriveMarker SV. */
  const useSmoothWorkletPath = isTripActiveMap && !DRIVE_CORE_V2;
  const driveMarker = useDriveMarker(DRIVE_CORE_V2 && isTripActiveMap, getTripActive);
  const driveCore = useDriveCore({
    ...
    onPoseAfterMatch: (out) => {
      ...
      const feedSpeedMs = !isNavigatingRef.current || out.isMoving
        ? Math.max(0.12, Math.min(52, Math.max(hudKmh, rawGpsKmhRef.current) / 3.6))
        : 0;
  ...
  useEffect(() => {
    ...
    driveCoreProcessRef.current = (lat, lng, accuracy, timestamp, gpsSpeedMs) => {
      ...
      const feedSpeedMs = isFreeDriveTick || out.isMoving
        ? Math.max(0.12, Math.min(52, Math.max(hudKmh, dopplerKmhTick) / 3.6))
        : 0;
      driveMarker.pushTarget({
        lat: out.pose.lat,
        lng: out.pose.lng,
  ...
  }, [driveCore, driveMarker, publishUserLocation, recordDrivingTracePoint, headingForDriveMarker, pushTripCameraFromApply]);
  ...
  /** V2: kamera z ticka GPS (driveCoreProcessRef), nie z useAnimatedReaction 60 FPS — freeze mapy. */
  ...
  /** V2: cruise + kamera ~5 Hz (bez useAnimatedReaction 60 FPS). */
  useEffect(() => {
    if (!DRIVE_CORE_V2 || !isTripActiveMap) return;
    const id = setInterval(() => {
      const now = Date.now();
      if (now - lastCamPushFromSmoothRef.current >= 180) {
        const la = driveMarker.lat.value;
        const ln = driveMarker.lng.value;
        const hdg = driveMarker.heading.value;
        if (Number.isFinite(la) && Number.isFinite(ln)) {
          lastCamPushFromSmoothRef.current = now;
          pushTripCameraFromApply(la, ln, Number.isFinite(hdg) ? hdg : 0);
        }
      }
      const gpsAge = Date.now() - lastGpsTickAtRef.current;
      if (gpsAge < 180 || gpsAge > 5000) return;
      const kmh = speedKmhRef.current;
      if (kmh >= 2.5) {
        driveMarker.setCruiseSpeed(Math.max(0.12, Math.min(52, kmh / 3.6)));
      }
    }, 100);
    return () => clearInterval(id);
  }, [isTripActiveMap, driveMarker, pushTripCameraFromApply]);

  // Android: między tickami GPS utrzymuj HUD prędkości (bez czekania na publishSpeed).
  useEffect(() => {
    if (!isTripActiveMap) return;
    const id = setInterval(() => {
      const gpsAgeMs = lastGpsTickAtRef.current > 0
        ? Date.now() - lastGpsTickAtRef.current
        : 0;
      if (gpsAgeMs < 700 || gpsAgeMs > 45_000) return;
      const hudKmh = speedKmhRef.current;
      if (hudKmh >= 2) {
        emitSpeedometerKmh(hudKmh);
      } else if (hudKmh <= 0.5) {
        emitSpeedometerKmh(0);
  ...
  }, [isNavigating, isDriving, userLocation, exitDrivingMode, setFollowMode, recenterTo, getMatchedPoints, bumpMatchedFreshness, resetSnapState, mapMatchCoord, drivingSnap, startTrip, recordDrivingTracePoint, applyRoadMatchPoints, resyncSnapAfterRoadGeometry, setSnapPoints, applyTripPosition]);
  ...
  const { start: startGPS, stop: stopGPS } = useAdaptiveGPS({
    ...
    onLocation: useCallback((loc) => {
      ...
      if (DRIVE_CORE_V2 && tripActiveNow) {
        ...
        // Bramka V2 odrzuciła fix — NIE kończ całego onLocation (inaczej free-drive =
        // brak HUD, brak gpsForceActive, marker stoi). Lekki fallback bez legacy.
        if (!gpsForceActiveRef.current) {
          gpsForceActiveRef.current = true;
          setGpsForceActive(true);
        }
        ...
        const fbLng = Number.isFinite(drLngRef.current) && drLngRef.current !== 0
          ? drLngRef.current
          : rawLng0;
        const fbHdg = Number.isFinite(lastHeadingRef.current)
      ...
      if (!isNavigatingRef.current) {
        ...
        if (navPts.length > 1) {
          ...
          const navCompassFallback = kmh < TRIP_COMPASS_HEADING_MAX_KMH ? navRaw : null;
          const navHdg = resolveUnifiedHeading({
            snapHeading: null,
  ...
  useEffect(() => {
    ...
    const rawLat = drFresh ? drLatRef.current : fallbackLoc?.latitude;
    const rawLng = drFresh ? drLngRef.current : fallbackLoc?.longitude;
    if (!Number.isFinite(rawLat) || !Number.isFinite(rawLng)) return;

    const navPts = navRouteRef.current?.points;
    const routePts = navPts?.length ? navPts : routePointsRef.current;
    const snapped = routePts.length
      ? snapToRoute(rawLat, rawLng, routePts, NAV_ROUTE_SNAP_M)
      : { latitude: rawLat, longitude: rawLng };
    ...
    lastRerouteLocRef.current  = { lat: snapped.latitude, lng: snapped.longitude };
    setRerouteOrigin({
