...
import { ActivityIndicator, Alert, RefreshControl, ScrollView, Switch, TouchableOpacity, View } from 'react-native';
...
export function InventoryScreen({ userId }: { userId?: string }) {
  ...
  return <ScrollView style={{ flex: 1, backgroundColor: theme.bg }} contentContainerStyle={{ padding: 18, paddingTop: 58, paddingBottom: 80 }} refreshControl={<RefreshControl refreshing={loading} onRefresh={load} tintColor="#f21933" />}>
    ...
    <Text style={{ color: '#ff5368', fontFamily: 'Manrope_600SemiBold', fontSize: 12, letterSpacing: 1 }}>VROOM INVENTORY</Text><Text style={{ color: theme.text, fontFamily: 'Manrope_600SemiBold', fontSize: 29, fontWeight: '900', marginTop: 8 }}>{isOwner ? 'Mój ekwipunek' : `Ekwipunek @${data?.user.username || ''}`}</Text><Text style={{ color: theme.textDim, marginTop: 8, lineHeight: 20 }}>Wszystkie itemy zdobyte za Nitro, w sklepie i nadane przez VROOM.</Text>
    {isOwner && data && <View style={{ marginTop: 18, padding: 15, borderRadius: 16, borderWidth: 1, borderColor: theme.border, backgroundColor: theme.surface, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}><View style={{ flex: 1 }}><Text style={{ color: theme.text, fontWeight: '800' }}>Publiczny ekwipunek</Text><Text style={{ color: theme.textDim, fontSize: 12, marginTop: 4 }}>Inni mogą zobaczyć Twoją kolekcję.</Text></View><Switch value={data.public} disabled={pending === 'privacy'} onValueChange={setPrivacy} trackColor={{ true: '#f21933' }} /></View>}
    ...
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, marginTop: 22, marginBottom: 18 }}><TouchableOpacity onPress={() => setFilter('all')} style={{ paddingHorizontal: 14, paddingVertical: 10, borderRadius: 99, backgroundColor: filter === 'all' ? '#f21933' : theme.surface, borderWidth: 1, borderColor: filter === 'all' ? '#f21933' : theme.border }}><Text style={{ color: theme.text, fontWeight: '800', fontSize: 12 }}>Wszystkie</Text></TouchableOpacity>{categories.map((category) => <TouchableOpacity key={category} onPress={() => setFilter(category)} style={{ paddingHorizontal: 14, paddingVertical: 10, borderRadius: 99, backgroundColor: filter === category ? '#f21933' : theme.surface, borderWidth: 1, borderColor: filter === category ? '#f21933' : theme.border }}><Text style={{ color: theme.text, fontWeight: '800', fontSize: 12 }}>{labels[category] || category}</Text></TouchableOpacity>)}</ScrollView>
    <View style={{ gap: 13 }}>{items.map((item) => { const active = data?.equipped?.[item.category] === item.id; const previewUri = normalizeMediaUri(item.previewUrl || item.assetUrl); return <View key={item.id} style={{ overflow: 'hidden', borderRadius: 20, borderWidth: 1, borderColor: active ? '#38d99666' : theme.border, backgroundColor: theme.surface }}>
      {item.category === 'map_vehicle_3d' && selected3d?.id === item.id ? <VehicleModelPreview3D item={item} height={220} isDark={isDark} /> : <TouchableOpacity disabled={item.category !== 'map_vehicle_3d'} onPress={() => setSelected3d(item)} style={{ height: 170, alignItems: 'center', justifyContent: 'center', backgroundColor: isDark ? '#111115' : '#e7e7ea' }}>{previewUri ? <Image source={{ uri: previewUri }} style={{ width: '100%', height: '100%' }} contentFit={item.category === 'avatar_frame' ? 'contain' : 'cover'} cachePolicy="memory-disk" /> : <MaterialCommunityIcons name="package-variant-closed" size={44} color={theme.textDim} />}{item.category === 'map_vehicle_3d' && <View style={{ position: 'absolute', right: 10, bottom: 10, paddingHorizontal: 10, paddingVertical: 7, borderRadius: 99, backgroundColor: '#08080bcc' }}><Text style={{ color: '#fff', fontSize: 12, fontWeight: '900' }}>PODGLĄD 3D</Text></View>}</TouchableOpacity>}
      <View style={{ padding: 16 }}><View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}><View style={{ flex: 1 }}><Text style={{ color: '#ff5368', fontSize: 12, fontWeight: '900', letterSpacing: 1 }}>{labels[item.category] || item.category}</Text><Text style={{ color: theme.text, fontFamily: 'Manrope_600SemiBold', fontSize: 16, fontWeight: '900', marginTop: 5 }}>{item.name}</Text></View>{active && <View style={{ paddingHorizontal: 9, paddingVertical: 6, borderRadius: 99, backgroundColor: '#38d99622' }}><Text style={{ color: '#38d996', fontSize: 12, fontWeight: '900' }}>AKTYWNY</Text></View>}</View><Text style={{ color: theme.textDim, fontSize: 12, lineHeight: 17, marginTop: 8 }}>{item.tagLine || item.description || 'Item VROOM'}</Text>{item.sources?.[0] && <Text style={{ color: theme.textDim, fontSize: 12, marginTop: 10 }}>{item.sources[0].label} · {new Date(item.sources[0].grantedAt).toLocaleDateString('pl-PL')}</Text>}{isOwner && equipable.has(item.category) && <TouchableOpacity disabled={pending === item.id || item.active === false} onPress={() => equip(item)} style={{ marginTop: 14, minHeight: 44, borderRadius: 99, alignItems: 'center', justifyContent: 'center', backgroundColor: active ? 'transparent' : '#f21933', borderWidth: 1, borderColor: active ? theme.border : '#f21933' }}>{pending === item.id ? <ActivityIndicator color="#fff" /> : <Text style={{ color: theme.text, fontWeight: '900', fontSize: 12 }}>{active ? 'Zdejmij' : 'Użyj itemu'}</Text>}</TouchableOpacity>}</View>
    </View>; })}</View>
