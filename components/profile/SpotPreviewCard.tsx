...
export default function SpotPreviewCard({ spot, isOwner = false, onPress, onDeleted }: Props) {
  ...
  return (
    <View style={{ width: '48%', marginBottom: 10, position: 'relative' }}>
      <TouchableOpacity
        style={{ backgroundColor: theme.surface3, borderRadius: 12, overflow: 'hidden', borderWidth: 1, borderColor: theme.border }}
        onPress={onPress} activeOpacity={0.8}
      >
        {thumb ? (
          <Image source={{ uri: thumb }} style={{ width: '100%', height: 110 }} resizeMode="cover" />
        ) : (
          <View style={{ width: '100%', height: 110, backgroundColor: theme.surface4, justifyContent: 'center', alignItems: 'center' }}>
            <MaterialIcons name="place" size={28} color={theme.primary} />
          </View>
        )}
        {deleting && (
          <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: '#000000aa', justifyContent: 'center', alignItems: 'center', borderRadius: 12 }}>
      ...
      {isOwner && (
        <TouchableOpacity style={{ position: 'absolute', top: 6, right: 6, backgroundColor: '#00000060', borderRadius: 12, padding: 2 }} onPress={() => setMenuOpen(true)}>
      ...
      <Modal visible={menuOpen} transparent animationType="fade" onRequestClose={() => setMenuOpen(false)}>
        <TouchableWithoutFeedback onPress={() => setMenuOpen(false)}>
          <View style={{ flex: 1, backgroundColor: theme.overlay, justifyContent: 'flex-end' }}>
            <TouchableWithoutFeedback>
              <View style={{ backgroundColor: theme.surface3, borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, paddingBottom: 34 }}>
                ...
                <View style={{ height: 1, backgroundColor: theme.border2, marginVertical: 4 }} />
                ...
                <TouchableOpacity style={{ marginTop: 8, paddingVertical: 14, alignItems: 'center', backgroundColor: theme.surface4, borderRadius: 10 }} onPress={() => setMenuOpen(false)}>
