  ...
  return (
    <View style={{
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, padding: 10, paddingBottom: 6 }}>
        ...
        <Text style={{ fontFamily: 'Manrope_600SemiBold', fontSize: 12, color: isMe ? '#ffffff80' : theme.textDim, letterSpacing: 1, flex: 1 }}>TRASA</Text>
      ...
      {/* Info */}
      <Text style={{ fontFamily: 'Manrope_600SemiBold', fontSize: 12, color: isMe ? theme.onPrimary : theme.text, fontWeight: '700', marginHorizontal: 10, marginTop: 8 }} numberOfLines={1}>{data.name}</Text>
      <View style={{ flexDirection: 'row', gap: 12, marginHorizontal: 10, marginTop: 4, marginBottom: 8 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3 }}>
          ...
          <Text style={{ fontFamily: 'Manrope_600SemiBold', fontSize: 12, color: isMe ? '#ffffff80' : theme.textDim }}>{data.distance.toFixed(1)} km</Text>
        </View>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3 }}>
          <MaterialIcons name="place" size={11} color={isMe ? '#ffffff40' : theme.textDim} />
          <Text style={{ fontFamily: 'Manrope_600SemiBold', fontSize: 12, color: isMe ? '#ffffff60' : theme.textDim }}>{data.points.length} pkt</Text>
        </View>
      </View>
      ...
      >
        ...
        <Text style={{ fontFamily: 'Manrope_600SemiBold', fontSize: 12, color: theme.onPrimary, fontWeight: '700', letterSpacing: 0.5 }}>NAWIGUJ PO TEJ TRASIE</Text>
      </TouchableOpacity>
    </View>
  );
}