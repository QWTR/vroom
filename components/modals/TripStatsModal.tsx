...
import { TripRouteLine } from '../trips/TripRouteLine';
import { SUMMARY_SPEED_PALETTE } from '../../lib/tripRouteLine';
...
import { resolveTripSummaryPresentation } from '../../lib/tripSummaryPresentation';

const SCREEN_H = Dimensions.get('window').height;
type ServerSummary = { activityId: number; analysisStatus: string; telemetryQuality: any; availability: any; summary: any; routePoints?: any[]; insight?: any; personalRecords?: { kind: string; value: number }[]; premiumLocked?: boolean };
...
export function TripStatsModal({ visible, stats, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setSyncing(true);
      try {
        const response = await apiRequest<ServerSummary>(`/activity/summary/session/${encodeURIComponent(stats.tripSessionId!)}`);
        if (cancelled) return;
        ...
        attempt += 1;
        if (!cancelled && attempt < 10) timer = setTimeout(load, 1_300);
        else if (!cancelled) setSyncing(false);
      }
    };
    void load();
  ...
  const presentation = useMemo(
    () => stats ? resolveTripSummaryPresentation(stats, server) : null,
    [server, stats],
  );
  const points = useMemo(() => presentation?.points ?? [], [presentation]);
  ...
  const display = presentation ?? resolveTripSummaryPresentation(stats, null);
  const movingSec = display.movingSec;
  const stoppedSec = display.stoppedSec;
  const shortTrip = display.distanceKm < 1;
  const savedInHistory = server?.summary?.savedInHistory === true;
  ...
  const storyData = {
    ...
    movingSec,
    stoppedSec,
    avgSpeedKmh: display.avgSpeedKmh,
    maxSpeedKmh: display.maxSpeedKmh,
