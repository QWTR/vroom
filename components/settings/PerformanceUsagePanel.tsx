  ...
  return (
    <View style={{ marginTop: 12, padding: 14, borderRadius: 18, backgroundColor: cardBg, borderWidth: 1, borderColor: border }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 9 }}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ color: text, fontFamily: 'Manrope_600SemiBold', fontSize: 12, fontWeight: '800' }}>POMIAR AKTYWNY</Text>
          <Text style={{ color: textDim, fontFamily: 'Manrope_600SemiBold', fontSize: 12, marginTop: 3 }}>Lokalna probka co 60 sekund</Text>
        </View>
        <Text style={{ color: textDim, fontFamily: 'Manrope_600SemiBold', fontSize: 12 }}>{durationLabel(summary.trackedMs)}</Text>
      </View>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 13, gap: 8 }}>
        {metrics.map(metric => (
          <View key={metric.label} style={{ width: '31%', minWidth: 86, flexGrow: 1, padding: 10, borderRadius: 12, backgroundColor: '#00000018', borderWidth: 1, borderColor: border }}>
            <Text style={{ color: textDim, fontFamily: 'Manrope_600SemiBold', fontSize: 12, letterSpacing: 1 }}>{metric.label}</Text>
            <Text style={{ color: text, fontFamily: 'Manrope_600SemiBold', fontSize: 14, fontWeight: '900', marginTop: 5 }}>{metric.value}</Text>
          </View>
        ))}
      </View>
      ...
      {summary.routes.length > 0 ? (
        <View style={{ marginTop: 14 }}>
          <Text style={{ color: textDim, fontFamily: 'Manrope_600SemiBold', fontSize: 12, letterSpacing: 1, marginBottom: 7 }}>KOSZT EKRANOW</Text>
          {summary.routes.slice(0, 5).map(row => (
            <View key={row.route} style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 7, borderTopWidth: 1, borderTopColor: border }}>
              <Text numberOfLines={1} style={{ color: text, fontFamily: 'Manrope_600SemiBold', fontSize: 12, flex: 1 }}>{routeLabel(row.route)}</Text>
              <Text style={{ color: textDim, fontFamily: 'Manrope_600SemiBold', fontSize: 12 }}>{durationLabel(row.trackedMs)} · {row.averageFps.toFixed(0)} FPS</Text>
              <Text style={{ color: '#e33835', fontFamily: 'Manrope_600SemiBold', fontSize: 12, width: 42, textAlign: 'right' }}>-{row.batteryDropPct.toFixed(1)}%</Text>
            </View>
          ))}
        </View>
      ) : (
        <Text style={{ color: textDim, fontFamily: 'Manrope_600SemiBold', fontSize: 12, lineHeight: 16, textAlign: 'center', marginTop: 14 }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 12, gap: 8 }}>
        ...
        <Text style={{ color: textDim, fontFamily: 'Manrope_600SemiBold', fontSize: 12, lineHeight: 16, flex: 1 }}>
        ...
        <TouchableOpacity onPress={clear} style={{ paddingHorizontal: 10, paddingVertical: 8, borderRadius: 10, borderWidth: 1, borderColor: border }}>
          <Text style={{ color: '#e33835', fontFamily: 'Manrope_600SemiBold', fontSize: 12, fontWeight: '800' }}>WYCZYSC</Text>
        </TouchableOpacity>
      </View>
      {summary.samples > 0 && summary.batteryLevelPct == null ? (
        <Text style={{ color: '#FF9800', fontFamily: 'Manrope_600SemiBold', fontSize: 12, lineHeight: 16, marginTop: 10 }}>
